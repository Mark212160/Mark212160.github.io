...
window.onload = function() {
    ...
    $('body').on('click', function() {
        // Try to close a menu only when menu is open
        if(menuOpen) {
             ...
             menuItems.each(function(index) {
                setTimeout(function() {
                    menuItems.eq(index).animate({'opacity': 0}, 300, function() {
                        menuItems.eq(index).css({'dispaly': 'none'});
                    });
                }, delay + (50*index));
            });//Cant's use 'this' here since 'this' corresponds to <body>
